...
const RECIPES_POSTS_DIR = join(process.cwd(), 'src', 'data', 'recipes');

/**
 * Get the slugs of all available recipes posts
 */
export async function getAllPostSlugs(): Promise<Array<string>> {
	return readdirSync(RECIPES_POSTS_DIR);
}

/**
 * Get the frontmatter metadata for all available recipes posts
 */
export async function getAllPostsFrontMatter(): Promise<Array<FrontMatter>> {
	const files = readdirSync(RECIPES_POSTS_DIR);

	return files
		.map((slug) => {
			const source = readFileSync(join(RECIPES_POSTS_DIR, slug), 'utf8');
...
export async function getPost(slug: string): Promise<Post> {
	const raw = readFileSync(join(RECIPES_POSTS_DIR, `${slug}.md`)).toString();
